...
### What is NTA?
Network Traffic Analysis is the process of capturing, inspecting
and analyzing data as it flows through a network to gain complete
visibility and understand what is being communicated inside and
outside the network.
...
| Use Case | Description |
|----------|-------------|
| **Monitor network performance** | Identify slow networks, bandwidth issues and unusual spikes |
| **Detect abnormalities** | Sudden performance peaks, unusual protocols, unexpected connections |
| **Inspect suspicious communication** | DNS exfiltration, malicious file downloads, lateral movement |

### From a SOC Perspective NTA Helps With
| SOC Activity | How NTA Helps |
|-------------|---------------|
| **Threat Detection** | Identify C2 traffic, beaconing, data exfiltration |
| **Incident Response** | Reconstruct attacks by replaying captured traffic |
| **Alert Validation** | Verify and validate SIEM alerts with raw packet evidence |
...
| Type | Description | Example |
|------|-------------|---------|
| **LAN** (Local Area Network) | Connects devices within a limited area | Home, office, school building |
| **WAN** (Wide Area Network) | Connects multiple networks across large geographic areas | The Internet |
...
|-------------|-------------|---------|
| **Endpoint Devices** | Traffic generated by end-user machines | Laptops, workstations, servers, phones |
| **Intermediary Devices** | Traffic passing through routing/forwarding devices | Routers, switches, firewalls, proxies |

Both source types can be found within LAN and WAN environments.

---

### Common Malicious Traffic Patterns to Detect
| Pattern | Description | Protocol |
|---------|-------------|---------|
| **DNS Exfiltration** | Data hidden in DNS queries and responses | DNS (port 53) |
| **C2 Beaconing** | Regular outbound connections at fixed intervals | HTTP/S, DNS |
| **Lateral Movement** | SMB or RDP connections between internal hosts | SMB, RDP |
| **Malicious Downloads** | Suspicious file downloads over HTTP | HTTP (port 80) |
| **Port Scanning** | Many connection attempts across multiple ports | TCP/UDP |
| **Fragmentation Attack** | IP packets broken into tiny pieces to evade IDS | IP layer |
| **Large Data Transfers** | Unusually large outbound transfers | Any protocol |
...
| **Zeek** | Framework | Network monitoring and log generation |
| **Suricata** | IDS/IPS | Real-time traffic analysis and alerting |
| **Snort** | IDS | Signature-based traffic detection |
| **NetworkMiner** | GUI | Passive network forensics and file extraction |

---

### Fragmentation Attack — How IDS Evasion Works
Normal packet: [HEADER][MALICIOUS PAYLOAD]
↓ Fragmented
Fragment 1: [HEADER][MALI]
Fragment 2: [HEADER][CIOUS]
Fragment 3: [HEADER][ PAYLOAD]

IDS inspects each fragment individually:

. Fragment 1: MALI → not matching any signature
. Fragment 2: CIOUS → not matching any signature
. Fragment 3: PAYLOAD → not matching any signature

Reassembled at destination:

. Full payload: MALICIOUS PAYLOAD → now executes

**Defense:** Configure IDS to reassemble fragments before
inspection, or alert on excessive fragmentation.

---

### NTA SOC Checklist
✅ Monitor for DNS queries to unknown or newly registered domains
✅ Alert on large outbound data transfers
✅ Check for beaconing patterns (regular intervals to same IP)
✅ Monitor SMB and RDP between internal hosts
✅ Flag connections to known malicious IPs (threat intel)
✅ Alert on unusual ports or protocols
