...
- **Accountability** — The security principle of recording and
alerting on who did what, when and from where. Achieved through
logs, audit trails and SIEM monitoring
- **Authentication** — The process of proving an identity is genuine.
Methods include passwords, OTPs, biometrics and passkeys
- **Authorisation** — The process of determining what an
authenticated identity is allowed to do. Defines permissions and
access controls
- **Accountability** — Recording and alerting on who did what,
when and from where to create an audit trail
## B
...
automatically
- **CAPA** — Common Analysis Platform for Artifacts, a tool
developed by the FireEye Mandiant team that identifies capabilities
present in executable files including PE files, ELF binaries,
.NET modules, shellcode and sandbox reports
...
- **ELF Binary** — Executable and Linkable Format, the standard
executable file format used on Linux and Unix systems
## F
...
and HTTP
- **Host-Centric Log** — A log capturing events that occur within
or related to a specific host, generated by devices like Windows
and Linux machines, servers, etc. Examples: file access,
authentication attempts, process execution, registry changes,
PowerShell execution
- **Hybrid IDS** — A detection approach that combines signature-based
and anomaly-based detection to leverage the strengths of both methods
...
- **IAAA** — Identity, Authentication, Authorisation and
Accountability — the four core principles governing how access
...
- **Network-Centric Log** — A log generated when hosts communicate
with each other or access the internet, generated by devices like
firewalls, IDS/IPS and routers. Examples: SSH connections, FTP file
access, web traffic
- **Next-Generation Firewall (NGFW)** — An advanced firewall type
that provides threat protection, includes an intrusion prevention
system, identifies anomalies using heuristic analysis and decrypts
SSL/TLS traffic for inspection
...
- **OTP** — One-Time Password, a temporary single-use password
used as an authentication factor, commonly sent via SMS or
...
- **PE File** — Portable Executable, the standard executable file
format used on Windows systems (.exe, .dll, .sys files)
...
- **SIEM** — Security Information and Event Management system, the
core security solution used by SOC analysts. Collects logs from
various sources, standardizes their format, correlates them and
detects malicious activity using detection rules
- **Source Address** — In a firewall rule, the IP address of the
machine originating the traffic
- **Stateful Firewall** — A firewall type that recognizes traffic
by patterns, applies complex rules and monitors network connections
over time
- **Stateless Firewall** — A basic firewall type that performs simple
filtering without tracking previous connections, efficient for
high-speed networks
- **Signature-Based IDS** — A detection method that matches network
traffic against known attack patterns called signatures. Effective
against known threats but cannot detect new, unknown attacks
- **Signature** — A unique pattern associated with a specific known
attack, used by signature-based IDS to detect that attack
- **Snort** — One of the most widely used open-source IDS solutions,
developed in 1998. Uses both signature-based and anomaly-based
detection to identify known threats
- **Sandbox Report** — The output generated by running a suspicious
file in an isolated sandbox environment, recording its behavior
and actions
